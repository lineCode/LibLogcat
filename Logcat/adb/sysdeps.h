...
//!!static inline int unix_read(int fd, void* buf, size_t len) { return TEMP_FAILURE_RETRY(unix_read_interruptible(fd, buf, len)); }
static inline int unix_read(int fd, void* buf, size_t len) { return unix_read_interruptible(fd, buf, len); }

#undef   read
#define  read  ___xxx_read

// See the comments for the !defined(_WIN32) version of unix_write().
static inline  int  unix_write(int  fd, const void*  buf, size_t  len)
{
...
static inline int unix_lseek(int fd, int pos, int where) {
...
static inline int adb_shutdown(int fd, int direction = SHUT_RDWR) {
...
static inline int unix_read_interruptible(int fd, void* buf, size_t len) {
...
static inline int64_t adb_lseek(int fd, int64_t pos, int where) {
...
static inline int unix_isatty(int fd) {
    return isatty(fd);
}
#define  isatty  ___xxx_isatty
...
static inline int  adb_socket_accept(int  serverfd, struct sockaddr*  addr, socklen_t  *addrlen)
{
    int fd;
...
static inline int  adb_setsockopt( int  fd, int  level, int  optname, const void*  optval, socklen_t  optlen )
{
