...
static int _fh_file_writev(FH f, const adb_iovec* iov, int iovcnt) {
    ...
    for (int i = 0; i < iovcnt; ++i) {
        ssize_t rc = _fh_file_write(f, iov[i].iov_base, (int)(iov[i].iov_len));
        if (rc == -1) {
...
extern int adb_poll(adb_pollfd* fds, size_t nfds, int timeout) {
    ...
    int result = WSAPoll(sockets.data(), (ULONG)sockets.size(), timeout);
...
static int _fh_socket_write(FH f, const void* buf, int len) {
    ...
    if (result == SOCKET_ERROR) {
		...
		D("Tried to write %d but %d written", len, result);
        //CHECK_LE(result, len) << "Tried to write " << len << " bytes to " << f->name << ", but " << result << " bytes reportedly written";
    }
    return result;
}
...
// with GetProcAddress("GetAddrInfoW").
    ...
    if (connect(s, addrinfo->ai_addr, (int)(addrinfo->ai_addrlen)) == SOCKET_ERROR) {
...
static int _console_read(const HANDLE console, void* buf, size_t len) {
    for (;;) {
        ...
        if (!g_console_input_buffer.empty()) {
            const int bytes_read = (int)(std::min(len, g_console_input_buffer.size()));
...
static int _console_write_utf8(const char* const buf, const size_t buf_size, FILE* stream,
    ...
    DWORD written = 0;
    if (!WriteConsoleW(console, utf16.c_str(), (DWORD)utf16.length(), &written, nullptr)) {
        errno = EIO;
        return -1;
    }
